  ...
  return (
    <div className='min-h-screen bg-white py-12'>
      <div className='w-9/12 mx-auto space-y-8'>
        ...
        >
          <h1 className='text-3xl font-bold text-white'>🎨 Gradient Generator</h1>
          <div className='flex gap-4'>
            ...
            <button className='px-16 py-2 bg-rose-500 text-white rounded font-medium' onClick={genrateGradient}>Generate</button>
          </div>
        </div>
        <div className='grid grid-cols-4 gap-4'>
          {
            gradients.map((item, index) => (
              <div
                key={index}
                className='h-[180px] rounded-xl relative'style={{background: item.gradient}}>
                <button onClick={() => onCopy(item.css)} className='bg-black/50  hover:bg-black text-white rounded absolute bottom-3 right-3 text-sm py-1 px-2'>Copy</button>
              </div>
            ))
          }
        </div>
